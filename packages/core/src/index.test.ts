...
test("analyzeInputPackage decodes the real agentApi fixture with reversible transform evidence", async () => {
  ...
  const escapedSpacesBefore = original.match(/\\x20/gi)?.length ?? 0;
  const escapedSpacesAfter = transformed.transformedSource.match(/\\x20/gi)?.length ?? 0;
  const hexadecimalNumbersAfter = transformed.transformedSource.match(/\b0x[0-9a-f]+\b/gi)?.length ?? 0;
  const unicodeEscapesAfter = transformed.transformedSource.match(/\\u(?:[0-9a-f]{4}|\{[0-9a-f]+\})/gi)?.length ?? 0;
  ...
  assert.ok(transformed.transforms.includes("literal_notation_normalize"));
  assert.ok(transformed.transforms.includes("object_property_literal_restore"));
  ...
  assert.ok(escapedSpacesBefore >= 500);
  assert.equal(escapedSpacesAfter, 0);
  assert.equal(hexadecimalNumbersAfter, 0);
  assert.equal(unicodeEscapesAfter, 0);
  assert.ok(!transformed.transformedSource.includes("a13_0x41a2"));
  assert.ok(transformed.transformedSource.includes("You are the AI Canvas action planner."));
  assert.ok(transformed.transformedSource.includes("AGENT_PLANNER_PROMPT_MAX_CHARS = 46000"));
  assert.ok(transformed.transformedSource.includes('responseLanguage: "简体中文"'));
  assert.ok(transformed.transformedSource.includes("?.canvas"));
  assert.ok(!/\?\.\[['"]canvas['"]\]/.test(transformed.transformedSource));
