...
- `BrowserRunSummary` and `runtime_trace.executionBoundary` record queue backend, run attempt, max attempts, worker id, lease recovery, and retry policy so multi-instance scheduling remains auditable.
